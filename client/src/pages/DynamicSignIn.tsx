...
export function SignIn() {
  ...
  const { isAuthenticated, isReady, walletConnected, login } = useDynamicAuth();

  // Redirect to dashboard if already authenticated
  useEffect(() => {
    if ((isAuthenticated || walletConnected) && isReady) {
      navigate("/");
    }
  }, [isAuthenticated, walletConnected, navigate, isReady]);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 islamic-pattern bg-gradient-to-b from-[#e9e1ca] to-[#f5f0e5]">
      ...
      <Card className="w-full max-w-md border-[#e9e1ca] shadow-lg bg-white">
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-gray-600">
            Connect to access your ethical financial platform
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-center p-6">
            ...
            <div className="flex flex-col space-y-4">
              ...
              {/* Dynamic widget container - this will only display an empty div where the Dynamic widget will appear when triggered */}
      ...
      <div className="mt-10 text-center">
        <p className="text-sm text-gray-600 mb-2">
          Learn more about our ethical financial services
        </p>
        <Button
          variant="link"
          ...
          onClick={() => navigate("/")}
        >
          Explore Sanafi AI <ArrowRight className="ml-1 h-4 w-4" />
        </Button>
      </div>
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-6 max-w-xl">
        ...
        <div className="bg-white p-3 rounded-lg shadow-sm flex items-center justify-center w-24 h-12">
          <div className="text-xs text-gray-500 opacity-70">Dynamic</div>
        </div>
      </div>
    </div>
  );
